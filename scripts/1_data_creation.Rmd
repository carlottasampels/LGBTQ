...
names(dat)[names(dat) == "dem.age_likely"] <- "age"
...
```{r Combine for just one Variable for mdd bzw. gad }
dat <- dat %>%
  mutate(
    any_mdd_diagnosis = case_when(
      mhd.mdd == "MDD" ~ "MDD",
      cidid.mdd_diagnosis == "MDD diagnosis" ~ "MDD",
      phq9.sum_score >= 10 ~ "MDD",
      TRUE ~ "No MDD or NA"
    ),
    any_gad_diagnosis = case_when(
      mhd.gad == "GAD" ~ "GAD",
      cidia.diagnosis == "GAD diagnosis" ~ "GAD",
      gad7.sum_score >= 10 ~ "GAD",
      TRUE ~ "No GAD or NA"
    )
  )

sum(dat$any_mdd_diagnosis == "MDD")
sum(dat$any_gad_diagnosis == "GAD")
```
